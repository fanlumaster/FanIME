...
开启后，仅在竖排候选窗口中为候选项显示中英互译，每项最多两个简短释义。优先使用本地释义；本地未命中时可填写腾讯云 TMT 的 SecretId / SecretKey，由云端补全。凭据只保存在本机配置文件中。
...
设置 → 词库可切换全拼、五笔和英文词库，支持查询、新增、编辑、删除。全拼和英文支持批量导入；也可导入纯汉字词组（例如人名），由输入法生成拼音。用户词库可分别导出，便于备份和迁移。快捷短语的导入导出在实用功能页。
