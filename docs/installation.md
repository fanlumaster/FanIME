...
Releases 页面上有两类发布，都标记为 Pre-release，靠标题区分：

| 标题形式 | 来源 | 建议 |
|---|---|---|
| `v<版本号>（自动构建）` | 每次合并到 `main` 后由 CI 自动构建发布，未经人工挑选 | 想要最新修复、且能接受未经验证的改动时用 |
| `v<版本号>` | 人工挑选后发布 | 一般情况下用这个 |

GitHub 的发布只有 Latest / Pre-release / Draft 三种状态，没有自定义频道，因此两类共用同一个 Pre-release 列表。
