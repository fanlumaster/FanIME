...
std::vector<WordItem> QueryPrefix(const std::string &code, SchemeType scheme, int limit)
{
    ...
    {
        // Xiaohe "xnlm" -> "xiaolian": also search the shuangpin->quanpin
        // expansion so Chinese keyword pinyin stays reachable in shuangpin
        // mode, while the raw code keeps English/abbreviated-pinyin matching.
    ...
    for (const auto &prefix : prefixes)
    {
        ...
        constexpr const char *sql = "SELECT emoji, sort_order FROM emoji_pinyin "
                                    "WHERE key>=?1 AND key<?2 ORDER BY sort_order LIMIT ?3";
        ...
        {
            const auto *emoji = reinterpret_cast<const char *>(sqlite3_column_text(stmt.get(), 0));
            if (!emoji)
            {
                continue;
            }
            if (!seen.insert(emoji).second)
            {
                continue;
            }
            entries.push_back({emoji, sqlite3_column_int(stmt.get(), 1)});
        }
    }
    ...
    {
        results.emplace_back(prefixes.front(), entries[index].emoji,
