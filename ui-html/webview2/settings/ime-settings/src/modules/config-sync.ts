...
export function setupConfigSync(): void {
  ...
  window.chrome.webview.addEventListener('message', (event: Event & { data?: any }) => {
    ...
    if (payload.data?.voice_input && typeof payload.data.voice_input === 'object') {
      ...
      if (typeof payload.data.voice_input.mute_system_audio === 'boolean') applyToggleState('voiceMuteSystemAudio', payload.data.voice_input.mute_system_audio);
    }
    ...
    const diagnosticLog = payload.data?.general?.diagnostic_log ??
    ...
    if (typeof payload.data?.utility?.r_mode === 'boolean') {
      applyToggleState('rModeToggleBtn', payload.data.utility.r_mode);
    }
